...
impl IqmBackend {
    ...
    async fn submit_job(&self, request: &IqmJobRequest) -> Result<String, BackendError> {
        ...
        let job_response: IqmJobResponse = response.json().await.map_err(|e| {
            BackendError::Http(format!("Failed to parse job response: {}", e))
        })?;
    ...
    async fn wait_for_job(&self, job_id: &str) -> Result<IqmJobResult, BackendError> {
        ...
        for i in 0..max_polls {
            ...
            let result: IqmJobResult = response.json().await.map_err(|e| {
                BackendError::Http(format!("Failed to parse job result: {}", e))
            })?;
    ...
    fn pulse_to_circuit(&self, request: &ExecutePulseRequest) -> IqmCircuit {
        ...
        let instructions = request.target_qubits.iter().map(|&q| {
            IqmInstruction {
    ...
    async fn execute_pulse(
        ...
        for measurement in &measurements {
            for shot_result in &measurement.result {
                ...
                let bitstring: String = shot_result.iter()
                    .take(num_qubits)
